...
export default function Sidebar({ isOpen, setIsOpen }: SidebarProps) {
      ...
      try {
        ...
        if (session?.user) {
          ...
          if (profile) {
            ...
            setAdminName(session.user.email?.split('@')[0] || 'Superadmin')
          }
        }
      } catch (err) {
  ...
  const menuItems = [
    ...
    { name: 'Manajemen Produk', path: '/admin/products', icon: <Package size={18} /> },
    { name: 'Kategori Barang', path: '/admin/categories', icon: <Tags size={18} /> },
    { name: 'Riwayat Penjualan', path: '/admin/sales', icon: <ShoppingCart size={18} /> },
    { name: 'Catatan Pengeluaran', path: '/admin/expenses', icon: <Wallet size={18} /> },
    { name: 'Data Pelanggan', path: '/admin/customers', icon: <Users size={18} /> },
  ]
        ...
        <div className="p-4 border-t border-white/5 bg-slate-950/40 flex-shrink-0">
          <div className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/5 mb-3">
            ...
            <div className="text-left overflow-hidden">
              <p className="text-[10px] text-orange-400 font-bold uppercase tracking-wider leading-none">Superadmin</p>
              <p className="text-xs font-black text-white leading-tight truncate mt-1" title={adminName}>{adminName}</p>
            </div>
          </div>
